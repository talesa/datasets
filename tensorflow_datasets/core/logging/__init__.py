...
@wrapt.decorator
def as_dataset(function, builder, args, kwargs):
  ...
  decoders = {}
  if kwargs.get('decoders', None):
    for key, val in py_utils.flatten_nest_dict(kwargs['decoders']).items():
      decoders[key] = f'{val.__module__}.{val.__class__.__name__}'
