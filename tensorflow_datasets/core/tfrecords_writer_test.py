...
class WriterTest(testing.TestCase):
  ...
  def test_write_riegeli(self):
    """Writes 8 records in 5 shards.

    Number of records is evenly distributed (2-1-2-1-2).
    """
    path = os.path.join(self.tmp_dir, 'foo.riegeli')
    riegeli_format = file_adapters.FileFormat.RIEGELI
    to_write = [
        (1, b'a'),
        (2, b'b'),
        (3, b'c'),
    ...
    with mock.patch.object(
      ...
      shards_length, total_size = self._write(
          to_write, path, file_format=riegeli_format)
    self.assertEqual(shards_length, [2, 1, 2, 1, 2])
    self.assertEqual(total_size, 9)
    written_files, all_recs = _read_records(path, file_format=riegeli_format)
    written_index_files, all_indices = _read_indices(path)
    self.assertEqual(written_files,
                     ['foo.riegeli-0000%s-of-00005' % i for i in range(5)])
    self.assertEqual(all_recs, [
        [b'f', b'g'],
        [b'd'],
        [b'a', b'b'],
    ...
    self.assertEqual(
        written_index_files,
        ['foo.riegeli-0000%s-of-00005_index.json' % i for i in range(5)])
    self.assertLen(all_indices, 5)
